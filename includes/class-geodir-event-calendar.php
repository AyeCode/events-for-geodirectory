...
class GeoDir_Event_Calendar {
	...
	public static function display_calendar( $args = array(), $instance = array() ) {
		...
		$post_id      = apply_filters( 'geodir_event_calendar_widget_post_id', ( isset( $instance['id'] ) && ! empty( $instance['id'] ) ) ? absint( $instance['id'] ) : 0, $instance, $id_base );
...
<style>#gdwgt_<?php echo $identifier; ?> .gd-div-loader,#gdwgt_<?php echo $identifier; ?> #cal_title{display:none;}</style>
	...
	public static function ajax_calendar( $args = array(), $instance = array() ) {
		...
		} elseif ( isset( $instance['_week_start_day'] ) ) {
		...
		if ( ! isset( $_REQUEST['mnth'] ) ) {
			$_REQUEST['mnth'] = date_i18n( 'n' );
		}
		if ( ! isset( $_REQUEST['yr'] ) ) {
			$_REQUEST['yr'] = date_i18n( 'Y' );
		}
		...
		if ( isset( $_REQUEST['id'] ) && ! empty( $_REQUEST['id'] ) ) {
			$query_args['p'] = absint( $_REQUEST['id'] );
		}
