...
class GeoDir_Event_AYI {
	...
	public static function geodir_ayi_widget_html( $post, $buttons = false, $gde = false ) {
		...
		$count        = self::count_interested( $post->ID, $gde );
	...
	/**
	 * Enqueue the "Are You Interested" js.
	 */
	public static function geodir_are_you_interested_js() {
		...
		<script type="text/javascript">
			jQuery(document).ready(function ($) {
				$('a.geodir-ayi-btn-rsvp').on("click", function (e) {
					e.preventDefault();
					var container = $('.ayi-html-wrap');
					var btnaction = $(this).attr('data-action');
					var type = $(this).attr('data-type');
					var postid = $(this).attr('data-postid');
					var gde = $(this).attr('data-gde');
					var data = {
						'action': 'geodir_ayi_action',
						'geodir_nonce': '<?php echo esc_js( $nonces['geodir_ayi_action'] ); ?>',
						'btnaction': btnaction,
						'type': type,
						'postid': postid,
						'gde': gde
					...
					$.post('<?php echo esc_url( $ajax_url ); ?>', data, function (response) {
						...
						if ( geodir_design_style() ) {
							echo 'if (typeof aui_init === "function") { aui_init(); }';
						}
						?>
					});
				});
