	...
	if ( ! empty( $_REQUEST['event_dates'] ) ) {
		...
		if ( is_array( $event_dates ) ) {
			...
			if ( $from_date != '' && $to_date == '' ) {
				...
				$dates .= wp_sprintf( __( '%1$s to %2$s', 'geodiradvancesearch' ), $from_date, $to_date );
			}
		} else {
...
function geodir_event_title_recurring_event( $title, $post_id = null ) {
	...
	if ( isset( $event_post->ID ) && $event_post->ID == $post_id && ! empty( $event_post->recurring ) ) {
		...
		} elseif ( is_single() && isset( $_REQUEST['gde'] ) && geodir_event_is_date( sanitize_text_field( $_REQUEST['gde'] ) ) && GeoDir_Event_Schedules::has_schedule( $post_id, sanitize_text_field( $_REQUEST['gde'] ) ) ) {
...
function geodir_event_super_duper_widget_init( $options, $super_duper ) {
	...
	if ( ! $gd_listings_widget_js && ! empty( $options['base_id'] ) && $options['base_id'] == 'gd_listings' ) {
		...
		wp_register_script( 'geodir-events-widget', GEODIR_EVENT_PLUGIN_URL . '/assets/js/widget' . $suffix . '.js', array( 'jquery' ), GEODIR_EVENT_VERSION );
