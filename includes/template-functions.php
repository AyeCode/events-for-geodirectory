...
function geodir_event_rss_item() {
	...
	if ( ! empty( $post ) && ! empty( $post->start_date ) && ! empty( $post->end_date ) && isset( $post->start_time ) && isset( $post->end_time ) && geodir_is_gd_post_type( $post->post_type ) ) {
		$start_date = mysql2date( 'D, d M Y H:i:s +0000', get_gmt_from_date( $post->start_date . ' ' . $post->start_time ), false );
		$end_date = mysql2date( 'D, d M Y H:i:s +0000', get_gmt_from_date( $post->end_date . ' ' . $post->end_time ), false );
